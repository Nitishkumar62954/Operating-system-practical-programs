...
{
int dup_at[n],dup_bt[n],tat[n],wt[n];for (inti=0;i <n; i++)
dup_at[i]=at[i];
...
processes[j]=processes[j+1];processes[j+1]=t;
...
if(dup_bt[j]<ct)
{int z=0;z=ct;
ct=ct+dup_bt[j];dup_bt[j]=0;
findTurnAroundTime(j,at,tat, ct);cout<<"\n~~~~~~\n";
cout<<z<<"|"<<"P"<<processes[j]<<"|"<<ct<<"\n";
//<<"\t"<<at[j]<<"\t"<<quantum1<<"\n";
cout<<"~~~~~~";
//cout<<"	1";
}
...
q=dup_at[i+1]-dup_at[i];ct=q+ct;
dup_bt[j]=dup_bt[j]-q;
 

if(dup_bt[j]==0)
{
findTurnAroundTime(j,at,tat,ct);dup_bt[j]=0;
//cout<<"\n	4";
}
cout<<"\n~~~~~~\n";
cout<<z<<"|"<<"P"<<processes[j]<<"|"<<ct<<"\n";
...
cout<<"EnterArrival_timeofprocessno"<<i+1<<":";cin>>arrival_time[i];
